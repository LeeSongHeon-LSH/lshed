...
`add` classifies exactly like `init`, appends to `lshed.yaml` without disturbing your comments, adds the parts to the current profile and to the managed set. Without keys it only lists. `status` shows the count as `창고 밖` (outside the shed). To put a part that is already in the shed into another profile, edit `profiles:` by hand; `add` tells you when that is the case.
