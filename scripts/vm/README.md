...
- Gemini, Copilot and Cursor are not installed here; only their file placement is verified locally (`LSHED_PROBE_ASK=0`, all green on 2026-09-08 with lshed 0.14.1). They are what the VM run is for.
- Latest full run (2026-09-08, lshed 0.14.1): Codex 17/17 and agy 16/16, every question answered on the first attempt.

### Running the model questions on this machine without touching the real config

The probe restores into `$HOME`-relative roots, so a scratch `HOME` keeps it away from `~/.codex`, `~/.gemini` and `~/.agents`. The tools then need their login files in that scratch HOME, nothing else:

```
H=/tmp/lshed-probe-home; mkdir -p $H/.codex $H/.gemini/antigravity-cli
cp -p ~/.codex/auth.json ~/.codex/config.toml ~/.codex/installation_id $H/.codex/
cp -p ~/.gemini/antigravity-cli/antigravity-oauth-token ~/.gemini/antigravity-cli/settings.json ~/.gemini/antigravity-cli/installation_id $H/.gemini/antigravity-cli/
HOME=$H LSHED_PROBE_DIR=$H/probe LSHED_BIN="node $PWD/dist/cli.js" scripts/vm/probe.sh codex
HOME=$H LSHED_PROBE_DIR=$H/probe LSHED_BIN="node $PWD/dist/cli.js" scripts/vm/probe.sh agy
