...
export async function init(ctx: Ctx, opts: { profile?: string; exclude?: string[] } = {}): Promise<InitResult> {
  ...
  if (await exists(manifestPath(ctx))) {
    throw new Error(`This shed is already initialized: ${manifestPath(ctx)}\n  To put new local things into it use 'lshed add'; to bring in another machine's setup use 'lshed restore' and then 'lshed save'.`);
  }

  // 세 종류로 가른다 (§3.7): 설치한 것 / 설치가 만들어낸 것 / 내가 쓴 것
  const d = await discover(ctx, opts.exclude);
  for (const [key, by] of d.generated) ctx.log(`  · ${key}  (generated by ${by} → skipped)`);
  ...
  const parts = [`${copied} parts`];
  if (res.packages.length) parts.push(`${res.packages.length} packages`);
  if (d.generated.size) parts.push(`${d.generated.size} generated skipped`);
  if (d.excluded.length) parts.push(`${d.excluded.length} excluded`);
  ctx.log(`\n${MANIFEST_FILE} written: ${manifestPath(ctx)}  (${parts.join(", ")}, profile "${profileName}")`);
  if (d.items.some((f) => f.kind === "package" && f.pkg.into)) ctx.log(`Fill in the install command (install:) for git packages in lshed.yaml yourself.`);
