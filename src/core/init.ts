...
import { type Ctx, manifestPath, INSTRUCTIONS, targetRel, MANIFEST_FILE, abs } from "./context.js";
...
export interface InitResult { manifest: Manifest; copied: number }

/**
 * 현재 환경 스캔 → 창고에 복사 + lshed.yaml 생성 + state 기록.
 * 로컬 파일은 읽기만 한다 (§7.2). 쓰는 곳은 창고와 <root>/lshed/ 뿐이다. 창고에 이미 lshed.yaml 이 있으면 거부.
 */
export async function init(ctx: Ctx, opts: { profile?: string } = {}): Promise<InitResult> {
