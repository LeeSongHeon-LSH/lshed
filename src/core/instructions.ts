...
/** 지침 파일의 절대 경로. 어댑터에 지침 파일이 없으면 null */
export function instructionsFile(ctx: Ctx): string | null {
  const name = ctx.adapter.instructionsFileName();
  return name ? path.join(ctx.adapter.root, name) : null;
}
