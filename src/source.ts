...
export function parseSource(raw: string): Source {
  ...
  if (idx <= 0) {
    throw new Error(`source has no scheme: "${raw}" (e.g. file:./skills/x, github:user/repo@v1)`);
  }
  const scheme = raw.slice(0, idx);
  const rest = raw.slice(idx + 1);
  switch (scheme) {
    case "file":
      if (!rest) throw new Error(`file: needs a path: "${raw}"`);
      return { scheme, path: rest };
    case "github": {
      const m = GITHUB_RE.exec(rest);
      if (!m) throw new Error(`not a github: source: "${raw}" (e.g. github:user/repo@v1.0#sub/path)`);
    ...
    default:
      if (!/^[a-z][\w-]*$/.test(scheme)) throw new Error(`unknown scheme "${scheme}": "${raw}"`);
...
export function cloneTarget(s: Source): { url: string; ref?: string } {
  ...
  throw new Error(`a file: source is not something to clone`);
}
