...
describe("lshed add", () => {
  it("init 직후엔 후보가 없고 exclude 는 기억된다", async () => {
    ...
    expect(logs.join("\n")).toContain("Nothing new outside the shed.");
  ...
  it("키 없이 부르면 후보만 보여주고 아무것도 바꾸지 않는다", async () => {
    ...
    expect(await add(ctx)).toEqual([]);
    const out = logs.join("\n");
    expect(out).toContain("4 outside the shed");
    for (const k of ["skills/beta", "agents/rev", "mcp/notion", "packages/tk  github:a/tk@"]) expect(out).toContain(k);
    expect(out).toContain("1 generated by package tk are left out");
  ...
  it("--all: 패키지는 출처+락으로, 스텁은 건너뛴다", async () => {
    ...
    expect((await status(ctx)).fresh).toEqual([]);
    expect(logs.join("\n")).toContain("4 added to the shed");
  });

  it("이미 창고에 있는 것과 없는 것을 구분해 알려준다", async () => {
    await appear();
    await expect(add(ctx, ["alpha"])).rejects.toThrow(/is already in the shed/);
    await expect(add(ctx, ["nope"])).rejects.toThrow(/was not found locally/);
  });
  ...
  it("창고에는 있지만 프로필이 안 쓰는 것은 힌트로만", async () => {
    ...
    await add(ctx);
    expect(logs.join("\n")).toContain('1 in the shed but not in profile "default": skills/alpha');
  });
});
